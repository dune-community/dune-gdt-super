...
from hapod.cellmodel.wrapper import (
    ...
    create_and_scatter_cellmodel_parameters,
)
...
def calculate_pod(result, product, mpi, tol, name):
    ...
    start = timer()
    if mpi.rank_world == 0:
        result, svals = pod(result, product=product, atol=0.0, rtol=0.0, l2_err=tol)
...
def cellmodel_pod(testcase, t_end, dt, grid_size_x, grid_size_y, pol_order, tol, logfile=None):
    ...
    for k in range(nc):
        modes[k], svals[k], elapsed_pod[k], total_num_snaps[k] = calculate_pod(
            snapshots[k], pfield_product, mpi, tol, "pfield_" + str(k)
        )
    ...
    if logfile is not None and mpi.rank_world == 0:
        for k in range(nc):
            logfile.write(
                "After the {}-th pfield POD, there are {} modes of {} snapshots left!\n".format(
                    k, len(modes[k]), total_num_snaps[k]
                )
            )
            logfile.write(
                "After the {}-th ofield POD, there are {} modes of {} snapshots left!\n".format(
                    k, len(modes[nc + k]), total_num_snaps[nc + k]
                )
            )
        logfile.write(
            "After the stokes POD, there are {} modes of {} snapshots left!\n".format(
...
if __name__ == "__main__":
    ...
    for i in range(len(modes)):
        modes[i], wins[i] = mpi.shared_memory_bcast_modes(
            modes[i], returnlistvectorarray=retlistvecs
        )

    # calculate errors
    errors = calculate_cellmodel_errors(
        modes, testcase, t_end, dt, grid_size_x, grid_size_y, pol_order, mu, mpi, logfile=logfile
    )
