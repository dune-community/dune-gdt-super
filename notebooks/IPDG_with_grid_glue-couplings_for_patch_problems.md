...
```python
def assemble_dirichlet_coupling_for_patches(ss, nn, ss_space, nn_space):
    coupling_grid = dd_grid.coupling_grid(ss, nn)
    local_grid = local_grids[ss]
    ...
    local_grid = local_grids[ss]
    diffusion = GridFunction(local_grid, kappa, dim_range=(Dim(d), Dim(d)))
    weight = GridFunction(local_grid, weight, dim_range=(Dim(d), Dim(d)))
