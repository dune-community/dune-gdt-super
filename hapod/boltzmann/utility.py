...
def create_boltzmann_parameters(count, min_param=0.0, max_param=8.0, seed=1):
...
def create_and_scatter_boltzmann_parameters(comm, min_param=0.0, max_param=8.0, seed=1):
    """ Samples all 3 parameters uniformly with the same width and adds random parameter combinations until
...
def create_boltzmann_solver(testcase, gridsize, mu, dt=-1):
...
def solver_statistics(solver, chunk_size, with_half_steps=True):
    num_time_steps = math.ceil(solver.t_end / solver.dt) + 1.0
    if with_half_steps:
...
def calculate_trajectory_error(final_modes, testcase, grid_size, mu, with_half_steps=True):
    error = 0
    solver = create_boltzmann_solver(testcase, grid_size, mu)
...
def calculate_total_projection_error(
    final_modes, testcase, grid_size, mu, total_num_snapshots, mpi_wrapper, with_half_steps=True
):
    trajectory_error = calculate_trajectory_error(final_modes, testcase, grid_size, mu, with_half_steps)
...
def calculate_error(
    final_modes, testcase, grid_size, mu, total_num_snapshots, mpi_wrapper, with_half_steps=True, logfile=None
):
    ...
    start = timer()
    err = calculate_total_projection_error(
        final_modes, testcase, grid_size, mu, total_num_snapshots, mpi_wrapper, with_half_steps
    )
