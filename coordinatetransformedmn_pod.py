...
def coordinatetransformedmn_pod(grid_size, l2_tol, testcase, logfile=None, selection_strategy="nth"):
    ...
    dt_nonadaptive = 1.0 / (math.sqrt(dimDomain) / dx + max_sigma_t)
    ...
    for interval_index in range(num_time_intervals):
        ...
        t_max = predicted_times[(interval_index + 1) * max_snaps_per_interval] if interval_index != num_time_intervals - 1 else t_end + 1
    ...
    nonlinear_snapshots, _, total_num_nonlinear_snapshots, _ = mpi.comm_world.gather_on_rank_0(
        nonlinear_snapshots, len(nonlinear_snapshots), num_modes_equal=False
    )
    ...
    svals = None
    if mpi.rank_world == 0:
        snapshots, svals, evals = pod(snapshots, atol=0.0, rtol=0.0, l2_err=l2_tol * np.sqrt(total_num_snapshots))
        nonlinear_snapshots, nonlinear_svals, nonlinear_evals = pod(
            nonlinear_snapshots, atol=0.0, rtol=0.0, l2_err=l2_tol * np.sqrt(total_num_snapshots)
        )
        selected_snapshots, selected_svals, selected_evals = pod(selected_snapshots, atol=0.0, rtol=0.0, l2_err=l2_tol * np.sqrt(total_num_snapshots))
        with open(f"{testcase}_grid_{grid_size}_procs_{mpi.size_world}_evals.log", "w") as evalfile:
            for eval in evals:
                evalfile.write(f"{eval:.15e}\n")
        with open(f"{testcase}_grid_{grid_size}_procs_{mpi.size_world}_nonlinear_evals.log", "w") as evalfile:
            for eval in nonlinear_evals:
                evalfile.write(f"{eval:.15e}\n")
        ...
        if logfile is not None:
            ...
            logfile.write(
                "After the POD with selected snapshots, there are "
                + str(len(selected_snapshots))
                + " modes of "
                + str(total_num_selected_snapshots)
                + " snapshots left!\n"
            )
            logfile.write(
...
if __name__ == "__main__":
    ...
    final_modes_selected, win_selected = mpi.shared_memory_bcast_modes(final_modes_selected, returnlistvectorarray=True)
    ...
    calculate_mean_errors(final_modes_selected, grid_size, mu, testcase, total_num_snapshots, mpi, logfile=logfile, selected=True)
    win.Free()
    win_selected.Free()
    logfile.close()
    if mpi.rank_world == 0:
